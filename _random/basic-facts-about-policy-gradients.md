...
1.  The *State-Value Function* $V^\pi(s) = \mathbb{E}\_{\pi}[G\_t \| s\_t=s]$ is the expected return from starting in state $s$ and following policy $\pi$.
2.  The *Action-Value Function* $Q^\pi(s,a) = \mathbb{E}\_{\pi}[G\_t \| s\_t=s, a\_t=a]$ is the expected return from starting in state $s$, taking action $a$, and then following policy $\pi$.
...
### The policy gradient method and theorem
